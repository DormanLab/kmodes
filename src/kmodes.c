...
/**
 * Initialize using Huang (1997)'s first initialization method.
 *
 * @param x[in]		data (n x p)
 * @param n[in]		number of observations
 * @param p[in]		number of coordinates
 * @param K[in]		number of clusters
 * @param seeds[out]	seeds (to be set)
 * @param sd_idx[out]	indices of chosen observations
 * @return		error status
 */
int kmodes_init_h97a(data_t **x, unsigned int n, unsigned int p, unsigned int K,
		data_t **seeds, unsigned int *sd_idx)
{
	UNUSED(n);
