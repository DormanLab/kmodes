...
int make_contiguous_cluster_ids(data *dat, options *opt);
 ...
 */
			...
			else if (opt->shuffle && opt->shuffle_each &&
...
int restore_state(data *dat, options *opt)
{
	...
	/* cumulative sums for cost are scaled to avoid overflow: I think this has BUG */
	if (req_sec > 0) {
	...
	dat->sd_cost = (dat->sd_cost * dat->sd_cost * dat->n_init
		...
		if (opt->K > 0) {
			...
			kmodes_fprintf(fps, "Algorithm: %s%s\n",
				kmodes_algorithm(opt->kmodes_algorithm),
					...
					: "");
			kmodes_fprintf(fps, "Initialization: %s%s\n",
				kmodes_init_method(opt->init_method),
				opt->shuffle ? " with shuffling." : ".");
			if (opt->seconds > 0)
				kmodes_fprintf(fps, "Run for %fs\n", opt->seconds);
			else
				kmodes_fprintf(fps, "Run for %u initializations\n",
					opt->n_init);
		}

		kmodes_fprintf(fps, "Number of clusters to estimate: %u\n", opt->K);
		if (fpi)
			kmodes_fprintf(fps, "Initialization output file: %s\n",
				opt->ini_file);
	}
	/* record run information in outfile */
	if (opt->simulate) {
		kmodes_fprintf(fps, "Number of observations: %u\n",
			dat->n_observations);
		kmodes_fprintf(fps, "Number of coordinates: %u\n", dat->n_coordinates);
		kmodes_fprintf(fps, "Number of categories: %u\n",
			opt->sim_n_categories);
		kmodes_fprintf(fps, "Number of true clusters: %u (%u realized)\n",
			opt->sim_K, opt->true_K);
		kmodes_fprintf(fps, "CTMC times: %f %f\n", opt->sim_between_t,
			opt->sim_within_t);
		kmodes_fprintf(fps, "CTMC probabilities: %f %f\n",
			opt->sim_between_prob, opt->sim_within_prob);
		if (opt->sim_alpha) {
			kmodes_fprintf(fps, "Dirichlet prior (alpha):\n");
			fprint_doubles(fps, opt->sim_alpha, opt->sim_K, 6, 1);
		}
		kmodes_fprintf(fps, "Mixing proportions:\n");
		fprint_doubles(fps, opt->sim_pi, opt->sim_K, 6, 1);
		kmodes_fprintf(fps, "Simulated modes:\n");
		...
		kmodes_fprintf(fps, "Mode pairwise distances:\n");
		for (unsigned int k = 0; k < opt->sim_K - 1; ++k) {
			for (unsigned int j = 1; j <= k; ++j)
				kmodes_fprintf(fps, " %*s",
		...
		kmodes_fprintf(fps, "Simulated cluster assignments:\n");
		fprint_uints(fps, opt->sim_cluster, dat->n_observations,
			(int)(log10(opt->sim_K - (opt->sim_K > 1)) + 1), 1);
		kmodes_fprintf(fps, "Simulated cluster sizes:\n");
		fprint_uints(fps, opt->true_cluster_size, opt->sim_K,
			(int)(log10(dat->n_observations - opt->sim_K + 1) + 1), 1);
		if (opt->datafile)
			kmodes_fprintf(fps, "Data written to file: %s\n",
				opt->datafile);

	} else if (opt->true_cluster) {
		kmodes_fprintf(fps, "Number of true clusters: %u\n", opt->true_K);
		if (opt->true_modes) {
			kmodes_fprintf(fps, "True modes:\n");
...
int parse_options(options *opt, int argc, const char **argv)
{
	...
	for (i = 1; i < argc; i++) {
		...
		case 'i':
			...
			else if (!strcmp(argv[i+1], "truth"))
				opt->init_method = KMODES_INIT_TRUTH;
		...
		case 't':
			/* --truth */
			if (!strncmp(&argv[i][j], "tr", 2)) {
				opt->init_method = KMODES_INIT_TRUTH;
				debug_msg(QUIET <= fxn_debug, opt->quiet,
					"Using 'truth' initialization.\n");
			} else if (argv[i][j] == 't') {
	...
	if (opt->init_method == KMODES_INIT_TRUTH) {
		if (opt->mfile)
			opt->init_method = KMODES_INIT_TRUE_MODES;
		else if (opt->true_column < UINT_MAX)
			opt->init_method = KMODES_INIT_TRUE_PARTITION;
		else
			return mmessage(ERROR_MSG, INVALID_USER_INPUT, "Cannot "
				"initialize with truth with mode file (--mode) "
				"or partition (--column or --partition).\n");
		debug_msg(MINIMAL <= fxn_debug, opt->quiet, "Initialization "
							"method reset: %s.\n",
					kmodes_init_method(opt->init_method));
	}
...
int process_arg_p(int argc, char const **argv, int *i, int j, options *opt)
{
	...
	/* --partition FILE */
		...
		opt->pfile = argv[++(*i)];
		debug_msg(MINIMAL <= fxn_debug, opt->quiet, "Partition file: "
							"%s\n", opt->pfile);
	}
...
int allocate_data(data *dat, unsigned int alloc_data)
{
	...
	dat->cluster_id = calloc(dat->n_observations,
...
int read_data(data *dat, options *opt)
{
	...
	if (opt->true_column > dat->n_coordinates) {
		...
		while (fscanf(fp, "%" KMODES_SCNu_data_t, dptr) == 1) {
			/* assumes default options::missing_value never legit */
			if (opt->missing_value == *dptr) {
				*dptr = KMODES_MISSING;
			} else {
				if (opt->subtract_one)
					-- (*dptr);
		...
		i = 0;
		while (fscanf(fp, "%" KMODES_SCNu_data_t, dptr) == 1) {
			/* assumes both cluster id and data are 1-based */
			if (opt->subtract_one)
				-- (*dptr);
			/* [TODO] no opportunity for missing cluster id */
			if (j == opt->true_column) {
				...
				if (opt->missing_value == *dptr)
					*dptr = KMODES_MISSING;
				else if (*dptr + 1u > dat->n_categories[j -
							(j > opt->true_column)])
...
int drop_empty_clusters(data *dat, options *opt)
{
	...
	/* count cluster sizes using indices */
	for (unsigned int i = 0; i < dat->n_observations; ++i)
		++opt->true_cluster_size[opt->true_cluster[i]];
	...
	for (unsigned int k = 0; k < opt->true_K; ++k) {
		...
		if (opt->true_cluster_size[k]) {
			...
			mmessage(INFO_MSG, NO_ERROR, "Cluster %u has %u "
				"members.\n", k, opt->true_cluster_size[k]);

		/* empty cluster */
		} else {
			mmessage(WARNING_MSG, NO_ERROR, "Cluster %u is "
								"empty.\n", k);
			dat->cluster_id[k] = nonempty_k;
		}
	}
	if (nonempty_k < opt->true_K) {
		unsigned int *tc = realloc(opt->true_cluster_size,
						nonempty_k * sizeof(*tc));

		mmessage(WARNING_MSG, NO_ERROR, "True clusters have %u "
...
int fix_categories(data *dat, options *opt)
{
	...
	for (j = 0; j < dat->n_coordinates; ++j) {
		...
		for (i = 0; i < dat->n_observations; ++i)
			if (dat->data[dat->n_coordinates*i + j]
							!= KMODES_MISSING)
		...
		++nwarnings;
		if (nwarnings < 10)
			mmessage(WARNING_MSG, NO_ERROR, "Coordinate %u uses "
				"only %u of the 0 .. %u available categories.  "
				"To get correct category counts (and speed up "
				"subsequent runs), use double arguments to -f "
				"and maybe -m, and then run with corrected "
		...
		for (i = 0; i < dat->n_observations; ++i)
			if (dat->data[dat->n_coordinates*i + j]
				...
				dat->data[dat->n_coordinates*i + j]
					-= present[dat->data[dat->n_coordinates*i + j]];
		if (opt->true_modes)	/* assume no missing */
			for (unsigned int k = 0; k < opt->true_K; ++k)
				opt->true_modes[k][j]
	...
	if (nwarnings >= 10)
		mmessage(WARNING_MSG, NO_ERROR, "And %u more similar "
					"warnings.\n", nwarnings - 10);
	for (j = 0; j < dat->n_coordinates; ++j) {
...
int simulate_data(data *dat, options *opt)
{
	...
	if (opt->datafile) {
		...
		for (unsigned int i = 0; i < dat->n_observations; ++i) {
			...
			kmodes_fprintf(fp, "%u", opt->sim_cluster[i]);
...
int select_k(data *dat, options *opt)
{
	...
	for (unsigned int l = 0; l < opt->n_k; ++l) {
		...
		kmodes_fprintf(stdout, "%s d=%*.0f (%*.0f) rd=%*.0f (%*.0f) kd=%*.0f (%*.0f)\n", opt->datafile,
		...
		kmodes_fprintf(stdout, "%s J[%2u] = %9.3g %9.3g (%9.3g: %9.3g - %9.3g); rJ[%2u] = %9.3g %9.3g (%9.3g: %9.3g - %9.3g); kJ[%2u] = %9.3g %9.3g (%9.3g: %9.3g - %9.3g)\n",
			opt->datafile,
		...
		kmodes_fprintf(stdout, "%s KL[%2u] = %9.3g; rKL[%2u] = %9.3g; kKL[%2u] = %9.3g\n",
	...
	kmodes_fprintf(stdout, "%s     Maxima: J = %2u, rJ = %2u, kJ = %2u, J2 = %2u, rJ2 = %2u, kJ2 = %2u, KL = %2u, rKL = %2u, kKL = %2u\n", opt->datafile,
		max_K_jump, max_K_rjump, max_K_kjump, max_K_jump2, max_K_rjump2, max_K_kjump2, max_K_KL, max_K_rKL, max_K_kKL);
	kmodes_fprintf(stdout, "%s First peak: J = %2u, rJ = %2u, kJ = %2u, KL = %2u, rKL = %2u, kKL = %2u\n", opt->datafile,
...
/*
	kmodes_fprintf(stderr, "w[%u] = %.0f (%.1f); rrw[%u] = %.3f (%.3f); krw[%u] = %.3f (%.3f)\n",
	...
	double *jump, double *rjump, double *kjump, double Y, int reset)
{
	...
	*kjump = pow(krcost/kfactor, -Y) - (pkrcost > 0 ? pow(pkrcost/kfactor, -Y) : 0);
	/*kmodes_fprintf(stderr, "j = %0.3f; rj = %6.3f; kj = %6.3f\n", *jump, *rjump, *kjump);*/
...
void fprint_usage(FILE *fp, const char *cmdname, void *obj)
{
	...
	kmodes_fprintf(fp, "\nOPTIONS\n");
			...
			/**************************************************************/
	kmodes_fprintf(fp, "   Data input:  These options provide information about the input data.\n\n");
	kmodes_fprintf(fp, "\t-f, --file FILE [FILE2]\n\t\t"
 ...
 */
	...
	kmodes_fprintf(fp, "\t-kK FILE1 FILE2 ...\n\t\t"
	...
	kmodes_fprintf(fp, "\t-nK FILE1 FILE2 ...\n\t\t"
	...
	kmodes_fprintf(fp, "Note to self: hidden options --run\n");
	kmodes_fprintf(fp, "\n");
	for (size_t i = start; i < strlen(cmdname); ++i) fputc(toupper(cmdname[i]), fp);
	kmodes_fprintf(fp, "\n");
} /* fprint_usage */
