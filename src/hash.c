 ...
 */
int add_sequence(hash **seq_count, unsigned int *seq, unsigned int length,
         size_t idx)
{
    hash *new;
    int first = 0;
 ...
 * @return        error status
 */
int add_seq_idx(hash *seq_count, unsigned int *seq, unsigned int length,
        size_t idx)
{
    hash *unit;
...
{
    return (b->count - a->count);  // change for decreasing order
}/* count_sort */
